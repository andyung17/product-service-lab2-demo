use dotenv::dotenv; // Import dotenv to load environment variables from an optional .env file.
...
async fn main() {
    dotenv().ok(); // Load .env when present; existing environment variables take precedence.
